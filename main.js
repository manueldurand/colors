...
document.getElementById('scores').addEventListener("click", afficheScores)
...
function afficheScores() {
  ...
  $.get(url, function(data) {
    // La requête GET a réussi, et les données sont disponibles dans le paramètre "data"
    
    console.log(data);
    var scoresList = data;
    
    // Sélectionnez l'élément HTML dans lequel vous voulez afficher le tableau
    var tableContainer = $("#liste");
    
    // Créez une balise <table> pour afficher les scores
    var table = $("<table>");
    
    // Créez l'en-tête du tableau
    var thead = $("<thead>");
    var headerRow = $("<tr>");
    var nameHeader = $("<th>").text("Nom");
    var scoreHeader = $("<th>").text("Score");
    headerRow.append(nameHeader, scoreHeader);
    thead.append(headerRow);
    table.append(thead);
    
    // Créez le corps du tableau avec les données des scores
    var tbody = $("<tbody>");
    
    // Parcourez chaque objet score dans le tableau
    scoresList.forEach(function(score) {
      // Créez une nouvelle ligne pour chaque score
      var row = $("<tr>");
      
      // Ajoutez une colonne pour le nom
      var nameCell = $("<td>").text(score.name);
      row.append(nameCell);
      
      // Ajoutez une colonne pour le score
      var scoreCell = $("<td>").text(score.score);
    ...
    // Ajoutez la table au conteneur HTML
    tableContainer.append(table);
  })
  .fail(function() {
    // La requête GET a échoué
    console.log("Erreur lors de la récupération des scores.");
  });
   
}

function updateScores(nom, points) {
  let newScore = {
    "name": "Henri",
    "score": "4"
  }
  var url = "scores.json";
  $.get("scores.json", data)
  let scoresList = data;
